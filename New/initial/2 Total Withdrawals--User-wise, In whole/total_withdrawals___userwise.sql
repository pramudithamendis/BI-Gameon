...
SET @cutoff := '2025-09-27 18:30:00'; insert into total_withdrawals_monthly_userwise(month_,user_id,username,total_withdrawal_amount,total_amount_lkr,total_transactions) SELECT DATE_FORMAT(w.created_at, '%Y-%m') AS month, u.id AS user_id, u.first_name AS username, SUM(w.withdrawal_amount) AS total_withdrawal_amount, SUM(w.amount_lkr) AS total_amount_lkr, COUNT(*) AS total_transactions FROM gaming_app_backend.user u JOIN gaming_app_backend.user_withdrawals w ON w.user_id = u.id WHERE w.is_active = 1 AND w.status = 'Approved' AND w.amount_lkr IS NOT NULL AND u.email NOT LIKE '%@gameonworld.ai%' and w.created_at >= @cutoff GROUP BY month, u.id, u.first_name ORDER BY month DESC, total_amount_lkr DESC;
