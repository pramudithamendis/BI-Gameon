...
CREATE TABLE total_withdrawals_weekly (
    id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
    year_week INT NOT NULL COMMENT 'Format: YYYYWW (e.g., 202544)',
...
SET @cutoff := '2025-09-27 18:30:00'; insert into total_withdrawals_weekly(year_week,week_start_date,week_end_date,total_withdrawal_usd,total_withdrawal_lkr) SELECT YEARWEEK(w.created_at, 1) AS year_week, MIN(DATE(w.created_at)) AS week_start_date, MAX(DATE(w.created_at)) AS week_end_date, SUM(w.withdrawal_amount) AS total_withdrawal_usd, SUM(w.amount_lkr) AS total_withdrawal_lkr FROM gaming_app_backend.user u JOIN gaming_app_backend.user_withdrawals w ON w.user_id = u.id WHERE w.is_active = 1 AND w.status = 'Approved' AND u.email NOT LIKE '%@gameonworld.ai%' AND w.amount_lkr IS NOT NULL and w.created_at >= @cutoff GROUP BY YEARWEEK(w.created_at, 1) ORDER BY YEARWEEK(w.created_at, 1) DESC;
...
SET @cutoff := '2025-09-27 18:30:00'; insert into total_withdrawals_monthly(month_,total_withdrawal_usd,total_withdrawal_lkr) SELECT DATE_FORMAT(w.created_at, '%Y-%m') AS month, SUM(w.withdrawal_amount) AS total_withdrawal_usd, SUM(w.amount_lkr) AS total_withdrawal_lkr FROM gaming_app_backend.user u JOIN gaming_app_backend.user_withdrawals w ON w.user_id = u.id WHERE w.is_active = 1 AND w.status = 'Approved' AND u.email NOT LIKE '%@gameonworld.ai%' AND w.amount_lkr IS NOT NULL and w.created_at >= @cutoff GROUP BY DATE_FORMAT(w.created_at, '%Y-%m') ORDER BY DATE_FORMAT(w.created_at, '%Y-%m') DESC;
