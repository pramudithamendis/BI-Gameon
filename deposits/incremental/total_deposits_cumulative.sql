...
SET @previous_total := COALESCE(
    (SELECT value FROM total_deposits_cumulative WHERE date < @yesterday ORDER BY date DESC LIMIT 1),
    0
);

-- Get yesterday's new user count
SET @yesterday_count := COALESCE(
    (SELECT value FROM total_deposits_daily WHERE date = @yesterday),
    0
);
